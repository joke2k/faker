...
class TestEsMx:
    ...
    def test_state_abbr_are_unique(self, faker):
        abbreviations = EsMxAutomotiveProvider.license_plate_state_abbr
    ...
    def test_state_abbr_method(self, faker, num_samples):
        for _ in range(num_samples):
            abbr = faker.state_abbr()
            assert abbr in EsMxAutomotiveProvider.license_plate_state_abbr

    def test_state_method(self, faker, num_samples):
        names = set(EsMxAutomotiveProvider.license_plate_states.values())
        for _ in range(num_samples):
            assert faker.state() in names
