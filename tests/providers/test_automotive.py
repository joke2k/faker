    ...
    def test_vin(self, faker, num_samples):
        for _ in range(num_samples):
            ...
            front_part_weight = calculate_vin_str_weight(
                vin_number[:8], [8, 7, 6, 5, 4, 3, 2, 10]
            )
            rear_part_weight = calculate_vin_str_weight(
                vin_number[9:], [9, 8, 7, 6, 5, 4, 3, 2]
            )
...
class TestDeAt(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile(
        r"(?P<prefix>[A-Z]{1,2})-[1-9]{1}[0-9]{0,4} [A-Z]{1,3}"
    )
...
class TestEsCo(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern = re.compile(
        r"[A-Z]{3}\d{3}|[A-Z]{3}\d{2}[A-Z]|T\d{4}|[A-Z]{2}\d{4}|R\d{5}|S\d{5}"
    )
...
class TestEsEs:
    ...
    old_format_pattern: Pattern = re.compile(
        r"(?P<province_prefix>[A-Z]{1,2})\s\d{4}\s[A-Z]{2}"
    )
    ...
    def test_plate_old_format(self, faker, num_samples):
        for _ in range(num_samples):
            ...
            assert match
            assert (
                match.group("province_prefix") in EsEsAutomotiveProvider.province_prefix
            )
    ...
    def test_plate_format(self, faker, num_samples):
        for _ in range(num_samples):
            ...
            assert isinstance(plate, str)
            assert self.new_format_pattern.match(
                plate
            ) or self.old_format_pattern.match(plate)
...
class TestNlBe(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile(
        r"(\d{3}-[A-Z]{3})|" r"([A-Z]{3}-\d{3})|" r"([1-2]-[A-Z]{3}-\d{3})"
    )
...
class TestNlNl(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern = re.compile(
        license_plate_car_pattern.pattern
        + "|"
        + license_plate_motorbike_pattern.pattern,
    )
...
class TestPlPl:
    def test_License_plate(self, faker, num_samples):
        pattern: Pattern = re.compile(
            r"{patterns}".format(patterns="|".join(faker.license_plate_regex_formats()))
        )
        for _ in range(num_samples):
            plate = faker.license_plate()
...
class TestTrTr(_SimpleAutomotiveTestMixin):
    ...
    def perform_extra_checks(self, license_plate, match):
        ...
        assert all(
            letter in TrTrAutomotiveProvider.ascii_uppercase_turkish
...
class TestUkUa(_SimpleAutomotiveTestMixin):
    ...
    def test_prefix(self, faker):
        for _ in range(10):
            ...
            assert (
                temporary in UkUaAutomotiveProvider.license_region_data.get("Lviv")[0]
            )
