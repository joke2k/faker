...
def get_module_and_member_to_import(
    ...
    cls_repr = str(cls)
    cls_name = getattr(cls, "__name__", getattr(cls, "_name", cls_repr))
    module, member = cls.__module__, cls_name
    if cls_name is None:
        # ???????????
        qualified_type: list[tuple[str, ...]] = re.findall(
            r"([a-zA-Z_0-9]+)\.([a-zA-Z_0-9]+)", cls_repr
        )
...
def get_member_functions_and_variables(
    cls: type, include_mangled: bool = False
...
def main() -> None:
    classes_and_locales_to_use_for_stub: List[Tuple[type, str]] = []
    ...
    all_members: List[Tuple[UniqueMemberFunctionsAndVariables, Optional[str]]] = [
    ...
    # Use the accumulated seen_funcs and seen_vars
    # to remove all variables that have the same name as a function somewhere
        ...
        for func_name, func_value in mbr_funcs_and_vars.funcs.items():
            ...
            if sig.return_annotation not in [
                ...
                *BUILTIN_MODULES_TO_IGNORE,
                ...
                module, member = get_module_and_member_to_import(
                    sig.return_annotation, locale
                )
    ...
    for str_sig, comment, is_preceding_comment in signatures_with_comments:
        if comment is not None and is_preceding_comment:
            signatures_with_comments_as_str.append(f"# {comment}\n    {sig}")
        elif comment is not None:
            sig_without_final_ellipsis = str_sig.strip(" .")
            ...
            signatures_with_comments_as_str.append(str_sig)
