...
class Provider(BaseProvider):
    ...
    def chrome(cls):
        ...
        platforms = (
            "(%s) AppleWebKit/%s (KHTML, like Gecko) Chrome/%s.0.%s.0 Safari/%s" % (
                cls.linuxPlatformToken(
                ), saf, random.randint(
            13, 15), random.randint(800, 899), saf),
            "(%s) AppleWebKit/%s (KHTML, like Gecko) Chrome/%s.0.%s.0 Safari/%s" % (
                cls.windowsPlatformToken(
                ), saf, random.randint(
            13, 15), random.randint(800, 899), saf),
            "(%s) AppleWebKit/%s (KHTML, like Gecko) Chrome/%s.0.%s.0 Safari/%s" % (
                cls.macPlatformToken(
                ), saf, random.randint(
            13, 15), random.randint(800, 899), saf),
        )
    ...
    def firefox(cls):
        ver = (
            'Gecko/%s Firefox/%s.0' % (Date_Time.Provider.dateTimeBetween(
                datetime(2011, 1, 1)), random.randint(4, 15)),
            'Gecko/%s Firefox/3.6.%s' % (Date_Time.Provider.dateTimeBetween(
                datetime(2010, 1, 1)), random.randint(1, 20)),
            'Gecko/%s Firefox/3.8' % (
                Date_Time.Provider.dateTimeBetween(datetime(2010, 1, 1)), ),
        )

        platforms = (
            "(%s; %s; rv:1.9.%s.20) %s" % (cls.windowsPlatformToken(), cls.randomElement(
                cls.langs), random.randint(0, 2), random.choice(ver)),
            "(%s; rv:1.9.%s.20) %s" % (cls.linuxPlatformToken(),
                                       random.randint(
                                           5, 7), random.choice(ver)),
            "(%s; rv:1.9.%s.20) %s" % (cls.macPlatformToken(),
                                       random.randint(
                                           2, 6), random.choice(ver)),
        )
    ...
    @classmethod
    def safari(cls):
        saf = "%s.%s.%s" % (random.randint(
            531, 535), random.randint(1, 50), random.randint(1, 7))
        ...
        platforms = (
            '(Windows; U; %s) AppleWebKit/%s (KHTML, like Gecko) Version/%s Safari/%s' % (
                cls.windowsPlatformToken(), saf, ver, saf),
            '(%s rv:%s.0; %s) AppleWebKit/%s (KHTML, like Gecko) Version/%s Safari/%s' % (
                cls.macPlatformToken(
                ), random.randint(
            2, 6), cls.randomElement(cls.langs), saf, ver, saf),
            '(iPod; U; CPU iPhone OS %s_%s like Mac OS X; %s) AppleWebKit/%s (KHTML, like Gecko) Version/%s.0.5 Mobile/8B%s Safari/6%s' % (
                random.randint(3, 4), random.randint(0, 3), cls.randomElement(
                    cls.langs), saf, random.randint(3, 4), random.randint(111, 119), saf
            )
        )
    ...
    def opera(cls):
        ...
        platforms = (
            '(%s; %s) Presto/2.9.%s Version/%s.00' % (cls.linuxPlatformToken(), cls.randomElement(
                cls.langs), random.randint(160, 190), random.randint(10, 12)),
            '(%s; %s) Presto/2.9.%s Version/%s.00' % (cls.windowsPlatformToken(), cls.randomElement(
                cls.langs), random.randint(160, 190), random.randint(10, 12)),
        )
