...
class FactoryTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._provider = FooProvider()
        self._generator.add_provider(self._provider)

    def test_add_provider_gives_priority_to_newly_added_provider(self):
        self._generator.add_provider(BarProvider())
        self.assertEqual('barfoo', self._generator.format('foo_formatter'))

    def test_get_formatter_returns_callable(self):
        formatter = self._generator.get_formatter('foo_formatter')
    ...
    def test_get_formatter_returns_correct_formatter(self):
        self.assertEqual(self._provider.foo_formatter, self._generator.get_formatter('foo_formatter'))

    def test_get_formatter_throws_exception_on_incorrect_formatter(self):
        self.assertRaises(AttributeError, self._generator.get_formatter, 'barFormatter')

    def test_format_calls_formatter_on_provider(self):
        self.assertEqual('foobar', self._generator.format('foo_formatter'))

    def test_format_transfers_arguments_to_formatter(self):
        self.assertEqual('bazfoo!', self._generator.format('foo_formatter_with_arguments', 'foo', append='!'))

    def test_parse_returns_same_string_when_it_contains_no_curly_braces(self):
        self.assertEqual('fooBar#?', self._generator.parse('fooBar#?'))

    def test_parse_returns_string_with_tokens_replaced_by_formatters(self):
        self.assertEqual('This is foobar a text with " foobar "',
                         self._generator.parse('This is {{foo_formatter}} a text with "{{ foo_formatter }}"'))
    ...
    def test_magic_call_calls_format(self):
        self.assertEqual('foobar', self._generator.foo_formatter())

    def test_magic_call_calls_format_with_arguments(self):
        self.assertEqual('bazfoo', self._generator.foo_formatter_with_arguments('foo'))
